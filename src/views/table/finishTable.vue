        ...
        <template slot-scope="scope">
          ...
          <el-button  v-if="scope.row.orderStat>=3" size="small" @click="handleShowDetail(scope.row)">{{ $t('table.showDetail') }}
           ...
           <el-button type="primary" v-if="scope.row.orderStat==3" size="small" @click="handleConfirmOrder(scope.row)">{{ $t('table.confirmOrder') }}
            ...
            <el-button type="primary" v-if="scope.row.orderStat==3" size="small" @click="handleBackOrder(scope.row)">{{ $t('table.backOrder') }}
          </el-button>
          ...
          <el-button  type="primary" v-if="scope.row.orderStat==4" size="small" @click="handleEndDetail(scope.row)">{{ $t('table.endDetail') }}
          </el-button>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="140px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item :label="$t('table.endTime')" prop="timestamp">
          <el-date-picker v-model="temp.timestamp" type="date"  format="yyyy-MM-dd" placeholder="Please pick a date"/>
        </el-form-item>
...
import {endOrderSend,backOrderSend,confirmOrderSend,createOrder,getOrderList,orderPublish,orderCancelPublish,updateOrder,deleteOrder,getOrderDetail,confirmOrderDetail,getAllOrderDetail} from '@/api/order'
...
export default {
  ...
  methods: {
    ...
    getType(){
          ...
          supplierTypeKeyValue=undefined
...
            supplierTypeOptions.push(data[i])
    ...
    handleBackOrder(row){
      ...
      data.orderId = row.orderId;
         backOrderSend(data).then(response=>{
        this.$message({
        message: '退回成功',
        type: 'success'
          })
    ...
    handleEndDetail(row){
      var data = new Object()
      data.orderId = row.orderId;
      endOrderSend(data).then(response=>{
        this.$message({
        message: '确认成功',
        type: 'success'
          })
